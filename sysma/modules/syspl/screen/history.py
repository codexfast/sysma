...
from ..models.syspl import SysplHistory, SysplData
    ...
    def add_widgets(self):
        ...
        def item_selected(event):
            ...
            with Session(config.DB_ENGINE) as session:
                data = session.query(SysplData)\
                    .filter(SysplData.history_id==_id and SysplData.failed==False).all()
                ...
                if data:
                    ...
                    messagebox.showerror("Erro", f"SYSPL #{_id} - no row for \"sysplhistory\"", parent=self)
